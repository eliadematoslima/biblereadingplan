...
const toggleButton = document.getElementById("toggle-plan-btn");
...
async function loadReadingPlan() {
  const fileName = currentPlan === "traditional"
...
function updateToggleButton() {
  if (currentPlan === "traditional") {
    toggleButton.textContent = "📘 Plano Tradicional";
    toggleButton.classList.remove("chronological");
  } else {
    toggleButton.textContent = "📖 Plano Cronológico";
    toggleButton.classList.add("chronological");
  }
}

toggleButton.addEventListener("click", () => {
    currentPlan = currentPlan === "traditional" ? "chronological" : "traditional";
...
function updateProgress(readingPlan) {
  const total = 1189; // total oficial de capítulos da Bíblia
...
function updatePlanMeta(readingPlan) {
  ...
  const totalChapters = readingPlan.reduce((sum, day) => sum + day.readings.length, 0);
  meta.textContent = `${readingPlan.length} dias • ${totalChapters} capítulos`;
}

// ==============================
// RENDERIZAÇÃO ORIGINAL (SEM QUEBRA DE DESIGN)
// ==============================
function renderList(readingPlan) {
  ...
  for (let month = 1; month <= 12; month++) {
    ...
    monthTitle.textContent = getMonthName(month - 1);
    monthSection.appendChild(monthTitle);

    const monthItems = groupedMonths[month] || [];
    ...
    monthItems.forEach(item => {
      ...
      contentSpan.textContent = ` — ${item.readings.join(" • ")}`;

      text.appendChild(dateSpan);
      text.appendChild(contentSpan);

      // Checagem se o dia está completo
      const completed = item.readings.every(chapter => progress[chapter]);
      ...
      checkbox.addEventListener("change", () => {
          item.readings.forEach(chapter => {
              progress[chapter] = checkbox.checked;
          });
          ...
          // Atualiza o storage e a barra no topo sem resetar o DOM da tela
